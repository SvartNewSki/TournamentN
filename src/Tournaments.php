<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="/css/style.css" />
    <link rel="stylesheet" href="/css/tournaments.css">
    <title>Document</title>
  </head>
  <body>
    <div class="title_text"> RELEASED <span class="text_color"> TOURNAMENTS </span></div>
    <div class="title_subtext">Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Itaque earum rerum hic tenetur a sapiente delectus, ut aut <br>
        reiciendis voluptatibus maiores alias consequatur aut perferendis doloribus asperiores repellat</div>
    <div class="rectangle">
        <img src="/img/Rectangle 4.png" alt="rectangle">
        <div class="pyg-block">
          <div class="title">PICK YOUR GAME</div>
          <div class="subtitle">PICK YOUR GAME</div>
        </div>
        <div class="game_pics">
          <img src="/img/lEAGUE OF LEGENDS.png" alt="LoL" class="test">
          <img src="/img/Call of duty.png" alt="CoD" class="game">
          <img src="/img/Valorant.png" alt="val" class="game">
          <img src="/img/csgo.png" alt="CSGO" class="game">
        </div>
    </div>
    </div>
  </body>
</html>
